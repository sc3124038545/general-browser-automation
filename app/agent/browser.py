...
class BrowserContextHelper:
    ...
    async def format_next_step_prompt(self) -> str:
        ...
        if browser_state and not browser_state.get("error"):
            ...
            # 检查反爬拦截警告
            if browser_state.get("anti_bot_warning"):
                anti_bot_warning = f"\n\n🚨 重要警告：{browser_state.get('anti_bot_warning')}"
                logger.warning(f"[browser] 向LLM传递反爬警告")
        ...
        prompt = NEXT_STEP_PROMPT.format(
            url_placeholder=url_info + anti_bot_warning + elements_text,
